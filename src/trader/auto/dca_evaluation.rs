...
mod tests {
    ...
    fn test_config() -> DcaConfigSnapshot {
        DcaConfigSnapshot {
            enabled: true,
            max_count: 3,
            cooldown_minutes: 60,
    ...
    fn test_dca_triggers_on_sufficient_drop() {
        ...
        position.average_entry_price = 1.0;
        position.current_price = Some(0.8); // -20% drop
        position.entry_size_sol = 1.0;
        position.dca_count = 0;

        let eval = DcaEvaluation::evaluate(&position, test_config()).unwrap();

        assert!(eval.should_trigger);
        assert_eq!(eval.calculations.pnl_pct, -20.0);
    }
    ...
    fn test_dca_blocked_by_max_count() {
        let mut position = Position::default();
        position.average_entry_price = 1.0;
        position.current_price = Some(0.8);
        position.entry_size_sol = 1.0;
        position.dca_count = 3; // At limit

        let eval = DcaEvaluation::evaluate(&position, test_config()).unwrap();

        assert!(!eval.should_trigger);
        assert!(eval.reasons[0].contains("count limit"));
    }
    ...
    fn test_dca_blocked_by_insufficient_drop() {
        let mut position = Position::default();
        position.average_entry_price = 1.0;
        position.current_price = Some(0.95); // -5% drop (need -10%)
        position.entry_size_sol = 1.0;
        position.dca_count = 0;
        ...
        assert!(!eval.should_trigger);
        assert!(eval.reasons[0].contains("insufficient"));
    }
}
