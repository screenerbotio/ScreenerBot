...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
 ...
 println!("Expected 324 bytes, got {} bytes", pool_acc.data.len());
    return Ok(());
  }
 ...
 println!("DexScreener SOL price: {:.12}", api_sol_price);
                if vault_a_valid && vault_b_valid {
                  println!(
                    "Decoder would calculate price if vaults were accessible"
                  );
                } else {
                  println!(
 ...
 println!("FluxBeam pool parsing failed");
  ...
  if found_vaults.len() < 2 {
    println!(
 "Found {} vaults, need at least 2 for token pair",
      found_vaults.len()
    );
