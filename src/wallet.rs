...
impl WalletDatabase {
    ...
    pub fn get_recent_snapshots_sync(&self, limit: usize) -> Result<Vec<WalletSnapshot>, String> {
        ...
        let snapshot_iter = stmt.query_map(params![limit], |row| {
            ...
            let snapshot_time = DateTime::parse_from_rfc3339(&snapshot_time_str)
                .map_err(|_| rusqlite::Error::InvalidColumnType(
                    2, "Invalid snapshot_time".to_string(), rusqlite::types::Type::Text
                ))?
                .with_timezone(&Utc);
        ...
        for snapshot_result in snapshot_iter {
            snapshots.push(snapshot_result.map_err(|e| format!("Failed to parse snapshot row: {}", e))?);
        }
    ...
    pub fn get_monitor_stats_sync(&self) -> Result<WalletMonitorStats, String> {
        ...
        let total_snapshots: i64 = conn.query_row(
            "SELECT COUNT(*) FROM wallet_snapshots", [], |row| row.get(0)
        ).map_err(|e| format!("Failed to count snapshots: {}", e))?;

        // Get latest snapshot info
        let latest_info: Option<(String, String, f64, i64)> = conn.query_row(
            r#"
        ...
        ).optional().map_err(|e| format!("Failed to get latest snapshot: {}", e))?;
    ...
    pub fn get_token_balances_sync(&self, snapshot_id: i64) -> Result<Vec<TokenBalance>, String> {
        ...
        let balances_iter = stmt.query_map(params![snapshot_id], |row| {
            Ok(TokenBalance {
                id: Some(row.get(0)?),
        ...
        for balance_result in balances_iter {
            balances.push(balance_result.map_err(|e| format!("Failed to parse token balance row: {}", e))?);
        }
...
async fn collect_wallet_snapshot() -> Result<WalletSnapshot, String> {
    ...
    let token_accounts = rpc_client.get_all_token_accounts(&wallet_address).await
    ...
    for account_info in &token_accounts {
        ...
        let balance_ui = if let Some(decimals) = crate::tokens::decimals::get_cached_decimals(&account_info.mint) {
