...
impl Database {
    ...
    pub fn get_table_stats(&self) -> Result<TableStats, String> {
        ...
        let count_tokens: i64 = conn.query_row(
            "SELECT COUNT(*) FROM tokens",
            [],
            |row| row.get(0)
        ).unwrap_or(0);
        
        let count_dexscreener: i64 = conn.query_row(
            "SELECT COUNT(*) FROM data_dexscreener_pools",
            [],
            |row| row.get(0)
        ).unwrap_or(0);
        
        let count_geckoterminal: i64 = conn.query_row(
            "SELECT COUNT(*) FROM data_geckoterminal_pools",
            [],
            |row| row.get(0)
        ).unwrap_or(0);
        
        let count_rugcheck: i64 = conn.query_row(
            "SELECT COUNT(*) FROM data_rugcheck_info",
            [],
            |row| row.get(0)
        ).unwrap_or(0);
        
        let count_fetch_log: i64 = conn.query_row(
            "SELECT COUNT(*) FROM api_fetch_log",
            [],
            |row| row.get(0)
        ).unwrap_or(0);
...
impl TableStats {
    pub fn total_rows(&self) -> usize {
        self.tokens + self.dexscreener_pools + self.geckoterminal_pools + 
        self.rugcheck_info + self.fetch_log
    }
}
