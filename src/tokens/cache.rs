...
impl TokenDatabase {
    ...
    pub async fn get_all_tokens(&self) -> Result<Vec<ApiToken>, Box<dyn std::error::Error>> {
        let mut stmt = self.connection.prepare("SELECT * FROM tokens ORDER BY liquidity_usd DESC")?;
    ...
    pub fn get_token_by_mint(
        ...
        let mut stmt = self.connection.prepare("SELECT * FROM tokens WHERE mint = ?1")?;
    ...
    fn insert_or_update_token(&self, token: &ApiToken) -> SqliteResult<()> {
    ...
    pub fn get_stats(&self) -> Result<DatabaseStats, Box<dyn std::error::Error>> {
        let mut stmt = self.connection.prepare("SELECT COUNT(*) FROM tokens")?;
        let total_tokens: i64 = stmt.query_row([], |row| row.get(0))?;

        let mut stmt = self.connection.prepare(
            "SELECT COUNT(*) FROM tokens WHERE liquidity_usd > 100"
        )?;
