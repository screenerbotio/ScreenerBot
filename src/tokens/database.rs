...
use crate::tokens::types::{
    DexScreenerData, GeckoTerminalData, RugcheckData, 
    TokenMetadata, Token, TokenResult, TokenError, UpdateTrackingInfo,
    SecurityRisk, TokenHolder, DataSource, Priority, WebsiteLink, SocialLink,
};
...
impl TokenDatabase {
    ...
    pub fn new(path: &str) -> TokenResult<Self> {
        ...
        crate::tokens::schema::initialize_schema(&conn)
            .map_err(|e| TokenError::Database(e))?;
    ...
    pub fn upsert_token(&self, mint: &str, symbol: Option<&str>, name: Option<&str>, decimals: Option<u8>) -> TokenResult<()> {
    ...
    pub fn list_tokens(&self, limit: usize) -> TokenResult<Vec<TokenMetadata>> {
        ...
        let tokens = stmt.query_map(params![limit], |row| {
            Ok(TokenMetadata {
                mint: row.get(0)?,
                symbol: row.get(1)?,
                name: row.get(2)?,
    ...
    pub fn upsert_geckoterminal_data(&self, mint: &str, data: &GeckoTerminalData) -> TokenResult<()> {
        let conn = self.conn.lock()
            .map_err(|e| TokenError::Database(format!("Lock failed: {}", e)))?;
    ...
    pub fn upsert_rugcheck_data(&self, mint: &str, data: &RugcheckData) -> TokenResult<()> {
        ...
        let markets_json = data.markets.as_ref()
    ...
    pub fn get_tokens_by_priority(&self, priority: i32, limit: usize) -> TokenResult<Vec<String>> {
        ...
        let mints = stmt.query_map(params![priority, limit], |row| row.get(0))
            .map_err(|e| TokenError::Database(format!("Query failed: {}", e)))?;
        
        mints.collect::<Result<Vec<_>, _>>()
            .map_err(|e| TokenError::Database(format!("Failed to collect: {}", e)))
    }
    ...
    pub fn get_oldest_non_blacklisted(&self, limit: usize) -> TokenResult<Vec<String>> {
        ...
        let mints = stmt.query_map(params![limit], |row| row.get(0))
            .map_err(|e| TokenError::Database(format!("Query failed: {}", e)))?;
    ...
    pub fn is_blacklisted(&self, mint: &str) -> TokenResult<bool> {
        ...
        let mut stmt = conn.prepare("SELECT 1 FROM blacklist WHERE mint = ?1")
            .map_err(|e| TokenError::Database(format!("Failed to prepare: {}", e)))?;
    ...
    pub fn get_blacklist_reason(&self, mint: &str) -> TokenResult<Option<(String, String)>> {
        ...
        let mut stmt = conn.prepare("SELECT reason, source FROM blacklist WHERE mint = ?1")
            .map_err(|e| TokenError::Database(format!("Failed to prepare: {}", e)))?;
    ...
    pub fn get_full_token(&self, mint: &str) -> TokenResult<Option<Token>> {
        ...
        let preferred_source = crate::config::with_config(|cfg| {
            cfg.tokens.preferred_market_data_source.clone()
        });
    ...
    fn get_priority(&self, mint: &str) -> TokenResult<Priority> {
        ...
        let mut stmt = conn.prepare("SELECT priority FROM update_tracking WHERE mint = ?1")
            .map_err(|e| TokenError::Database(format!("Failed to prepare: {}", e)))?;
        
        let priority: i32 = stmt.query_row(params![mint], |row| row.get(0))
...
fn assemble_token(
    ...
    let (price_usd, price_sol, price_native, price_changes, market_metrics, volumes, txns, fetched_at) = 
        match market_data {
    ...
    let (mint_authority, freeze_authority, security_score, is_rugged, security_risks, top_holders, total_holders, creator_balance_pct, transfer_fee_pct) = 
    ...
    Token {
        ...
        is_blacklisted,
        priority,
        first_seen_at: DateTime::from_timestamp(metadata.created_at, 0).unwrap_or_else(|| Utc::now()),
