...
mod tests {
    ...
    async fn test_api_status() {
        ...
        // Don't assert since API might be down during testing
    }

    #[tokio::test]
    async fn test_token_analysis() {
        // Test with the sample token from the curl example
        let mint = "6WiJZTKLN4ZN4ncUbB7cZZFr2UTW3yF3pLeT2w23ZCdk";

        if let Ok(security_info) = analyze_token_security(mint).await {
            println!("Security Info: {:?}", security_info);
