...
mod tests {
    ...
    #[test]
    fn test_token_authorities_risk_levels() {
        // Test fully renounced token (safest)
        let safe_token = TokenAuthorities {
            mint_authority: None,
        ...
        assert_eq!(safe_token.get_risk_level(), TokenRiskLevel::Safe);
        assert!(safe_token.is_fully_renounced());
        assert!(safe_token.is_rug_safe());

        // Test token with only update authority (low risk)
        let low_risk_token = TokenAuthorities {
            mint_authority: None,
            freeze_authority: None,
            update_authority: Some("auth123".to_string()),
            is_token_2022: false,
            mint: "test".to_string(),
        };
        assert_eq!(low_risk_token.get_risk_level(), TokenRiskLevel::Low);
        assert!(!low_risk_token.is_fully_renounced());
        assert!(low_risk_token.is_rug_safe());

        // Test token with mint authority (medium risk)
        let medium_risk_token = TokenAuthorities {
            mint_authority: Some("auth123".to_string()),
            freeze_authority: None,
            update_authority: None,
            is_token_2022: false,
            mint: "test".to_string(),
        };
        assert_eq!(medium_risk_token.get_risk_level(), TokenRiskLevel::Medium);
        assert!(!medium_risk_token.is_rug_safe());

        // Test token with both mint and freeze authority (high risk)
        let high_risk_token = TokenAuthorities {
            mint_authority: Some("auth123".to_string()),
            freeze_authority: Some("auth456".to_string()),
            update_authority: Some("auth789".to_string()),
            is_token_2022: false,
            mint: "test".to_string(),
        };
        assert_eq!(high_risk_token.get_risk_level(), TokenRiskLevel::High);
        assert!(!high_risk_token.is_rug_safe());
    }

    #[test]
    fn test_authority_checks() {
        let token = TokenAuthorities {
            mint_authority: Some("auth123".to_string()),
            freeze_authority: None,
            update_authority: Some("auth789".to_string()),
            is_token_2022: true,
        ...
        assert!(!token.is_mint_disabled());
        assert!(token.is_freeze_disabled());
        assert!(!token.is_update_disabled());
        assert!(token.has_any_authority());
        assert!(!token.is_fully_renounced());
    }
}
