...
pub async fn update_token(
    ...
    match coordinator.acquire_dexscreener().await {
        Ok(_) => {
            match dexscreener::fetch_dexscreener_data(mint, db).await {
    ...
    match coordinator.acquire_geckoterminal().await {
        Ok(_) => {
            match geckoterminal::fetch_geckoterminal_data(mint, db).await {
    ...
    match coordinator.acquire_rugcheck().await {
        Ok(_) => {
            match rugcheck::fetch_rugcheck_data(mint, db).await {
    ...
    if !successes.is_empty() {
        let _ = db.mark_updated(mint, true);
    }
...
async fn update_critical_tokens(db: &TokenDatabase, coordinator: &RateLimitCoordinator) {
    ...
    println!("[UPDATES] Updating {} critical priority tokens", tokens.len());
    ...
    for mint in tokens {
        match update_token(&mint, db, coordinator).await {
            ...
            Ok(result) => {
                eprintln!("[UPDATES] Total failure for {}: {:?}", mint, result.failures);
            }
            Err(e) => {
