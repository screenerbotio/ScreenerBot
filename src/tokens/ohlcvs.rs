...
impl OhlcvService {
    ...
    pub async fn get_ohlcv_data(
        ...
        // Note: SOL coverage no longer needed - API now returns direct SOL prices
        ...
        match self.fetch_ohlcv_from_api(mint, &pool_address, limit).await {
            Ok(data_points) => {
                ...
                // Save to database (already SOL-denominated from API)
            ...
            Ok(sol_data) => {
                ...
                if is_debug_ohlcv_enabled() {
                    log(
                        ...
                        "API_SUCCESS",
                        &format!(
                            "✅ Retrieved {} SOL-denominated OHLCV data points via GeckoTerminal module",
                            sol_data.len()
                        )
                    );
                }

                // No conversion needed - API returns direct SOL prices
                Ok(sol_data)
            }
            Err(e) => {
                if is_debug_ohlcv_enabled() {
...
/// Note: SOL price coverage is no longer needed as we fetch direct SOL-denominated data
/// This function is kept for backward compatibility but is now a no-op
pub async fn ensure_sol_price_coverage() -> Result<(), String> {
    // No longer needed - we fetch direct SOL-denominated OHLCV data
    Ok(())
}
