...
pub async fn get_ata_rent_lamports() -> Result<u64, ScreenerBotError> {
    ...
    logger::info(LogTag::Rpc, "Fetching ATA rent from RPC");
    ...
    match response {
            ...
            if response.status().is_success() {
                match response.json::<serde_json::Value>().await {
                    Ok(rpc_response) => {
                        ...
                        logger::warning(LogTag::Rpc, "RPC response missing result for ATA rent");
                    }
                    Err(e) => {
                        logger::warning(
                            LogTag::Rpc,
                ...
                logger::warning(LogTag::Rpc, "Rate limited on RPC");
            } else {
                logger::warning(
                    LogTag::Rpc,
        ...
        Err(e) => {
            logger::warning(LogTag::Rpc, &format!("Failed to connect to RPC: {}", e));
        }
    }
...
impl RpcRateLimiter {
        ...
        if self.consecutive_successes >= 5 {
            ...
            if self.consecutive_429s == 0 {
                ...
                if had_previous_429s {
                    logger::debug(LogTag::Rpc, "Rate limit backoff reset to normal");
                }
            } else {
                let backoff_factor = self.backoff_multiplier.powi(self.consecutive_429s as i32);
                let new_interval_ms =
    ...
    pub fn reset(&mut self) {
        ...
        self.consecutive_successes = 0;
        logger::debug(LogTag::Rpc, "Rate limiter reset");
    }
...
impl RpcClient {
    ...
    pub fn new() -> Self {
        Self::from_config().unwrap_or_else(|e| {
            logger::debug(LogTag::Rpc, &format!("Failed to load config: {}", e));
            logger::error(
                LogTag::Rpc,
    ...
    pub fn from_config() -> Result<Self, String> {
        ...
        for (i, url) in rpc_urls.iter().enumerate() {
            logger::debug(LogTag::Rpc, &format!("RPC URL {}: {}", i + 1, url));
        }
    ...
    pub fn url(&self) -> String {
        match self.current_url.lock() {
            Ok(url) => url.clone(),
            Err(_) => {
                logger::warning(LogTag::Rpc, "Failed to lock current_url - using first URL");
    ...
    pub fn rotate_to_next_url(&self) -> String {
        let next_url = match (self.current_url_index.lock(), self.current_url.lock()) {
            ...
            _ => {
                logger::warning(LogTag::Rpc, "Failed to rotate URL - lock contention");
                self.rpc_urls.get(0).unwrap_or(&"".to_string()).clone()
            }
        };
    ...
    pub fn get_next_url(&self) -> String {
        match self.current_url_index.lock() {
            ...
            Err(_) => {
                logger::warning(LogTag::Rpc, "Failed to get next URL - lock contention");
                self.rpc_urls.get(0).unwrap_or(&"".to_string()).clone()
            }
        }
    }
    ...
    pub fn get_current_url_index(&self) -> usize {
        match self.current_url_index.lock() {
            Ok(index) => *index,
            Err(_) => {
                logger::warning(LogTag::Rpc, "Failed to get URL index - lock contention");
                0
            }
        }
    }
    ...
    pub fn create_current_rpc_client(&self) -> Arc<SolanaRpcClient> {
        let current_url = self.url();
        logger::debug(LogTag::Rpc, "Creating client for current URL");
        let client =
            SolanaRpcClient::new_with_commitment(current_url, CommitmentConfig::confirmed());
        Arc::new(client)
    }
    ...
    pub fn create_client_for_url(&self, url: &str) -> Arc<SolanaRpcClient> {
        logger::debug(LogTag::Rpc, "Creating client for specific URL");
        let client =
        ...
        match &result {
            ...
            Err(e) => {
                ...
                if e.starts_with("rate_limit:") {
                    ...
                    logger::debug(LogTag::Rpc, "Rate limited on RPC for batch request");
                } else {
                    logger::debug(LogTag::Rpc, &format!("Failed to fetch accounts: {}", e));
                }
            }
        }
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    self.record_429_error(Some(&current_url));
                    logger::warning(LogTag::Rpc, "Rate limited on RPC for SOL balance");
                } else {
                    logger::error(
                        LogTag::Rpc,
            ...
            Err(e) => {
                ...
                if Self::is_rate_limit_error(&error_msg) {
                    ...
                    logger::error(LogTag::Rpc, &format!("Failed to get SOL balance: {}", e));
                }
                Err(())
            }
        };
    ...
    pub async fn get_latest_blockhash(&self) -> Result<Hash, ScreenerBotError> {
        ...
        logger::debug(LogTag::Rpc, "Fetching latest blockhash from RPC");
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                        ...
                        Ok(rpc_response) => {
                            ...
                            if let Some(error) = rpc_response.get("error") {
                                ...
                                if Self::is_rate_limit_error(&error_str) {
                                    ...
                                    logger::warning(
                                        LogTag::Rpc,
                                        "Rate limited on RPC for blockhash",
                                    );
                                } else {
                            ...
                            logger::warning(LogTag::Rpc, "No valid blockhash found in response");
                        }
                        Err(e) => {
                            logger::warning(
                                LogTag::Rpc,
                    ...
                    // Record 429 error for adaptive rate limiting
                    self.record_429_error(Some(&current_url));
                    logger::warning(LogTag::Rpc, "Rate limited on RPC for blockhash");
                } else {
                    logger::warning(
                        LogTag::Rpc,
                        &format!(
    ...
    pub async fn send_transaction(
        ...
        logger::info(LogTag::Rpc, "Sending transaction to RPC");
            ...
            .await
        {
                ...
                if response.status().is_success() {
                    if let Ok(rpc_response) = response.json::<serde_json::Value>().await {
                        ...
                        if let Some(error) = rpc_response.get("error") {
                            ...
                            logger::error(LogTag::Rpc, &format!("RPC error: {}", error_msg));
                    ...
                    self.record_429_error(Some(&current_url));
                    logger::warning(LogTag::Rpc, "Rate limited on RPC for transaction send");
                } else {
                    logger::error(
                        LogTag::Rpc,
            ...
            Err(e) => {
                ...
                if Self::is_rate_limit_error(&error_msg) {
                    ...
                    logger::error(LogTag::Rpc, &format!("Failed to send transaction: {}", e));
    ...
    /// Gets only NFT mints for a wallet (decimals=0, amount=1) - optimized for license verification
    /// Returns Vec<(mint_pubkey, decimals)> to avoid additional RPC calls
    pub async fn get_nft_mints_for_wallet(
        &self,
        wallet_address: &str,
        ...
        logger::debug(
            LogTag::License,
            &format!("Fetching NFTs for wallet from RPC: {}", current_url),
        );
                ...
                .await
            {
                    ...
                    if response.status().is_success() {
                        match response.json::<serde_json::Value>().await {
                            Ok(rpc_response) => {
                                if let Some(result) = rpc_response.get("result") {
                                    if let Some(value) = result.get("value") {
                                        if let Some(accounts) = value.as_array() {
                                            for account in accounts {
                                                // Extract NFT info (decimals=0, amount=1)
                                                if let Some(nft_mint) =
                                                    extract_nft_mint_if_valid(account)
                                                {
                            ...
                            Err(e) => {
                                logger::warning(
                                    LogTag::License,
                                    &format!("Failed to parse {} NFT response: {}", token_type, e),
                                );
                            }
                        }
                        ...
                        logger::warning(
                            LogTag::License,
                            &format!(
                                "Rate limited on RPC {} for {} NFT check",
                                current_url, token_type
                            ),
                        );
                    } else {
                        logger::warning(
                            LogTag::License,
                            &format!(
                                "RPC error status {} for {} NFT check: {}",
                                response.status(),
                                token_type,
                                current_url
                            ),
                ...
                Err(e) => {
                    logger::warning(
                        LogTag::License,
                        &format!(
                            "Failed to connect to RPC {} for {} NFT check: {}",
                            current_url, token_type, e
                        ),
                    );
                }
        ...
        // Record successful call if we got any NFTs
        ...
        logger::debug(
            LogTag::License,
            &format!(
                "Found {} total NFT mints for wallet={}",
                nft_mints.len(),
                wallet_address
            ),
        );
                                    ...
                                    if let Some(owner_str) = owner.as_str() {
                                        ...
                                        logger::info(
                                            ...
                                            &format!(
                                                ...
                                                mint, owner_str, is_token_2022, current_url
                                            ),
                                        );
                ...
                if response.status().is_success() {
                    if let Ok(rpc_response) = response.json::<serde_json::Value>().await {
                        if let Some(result) = rpc_response.get("result") {
                            ...
                            logger::info(
                                LogTag::Rpc,
                                &format!("Retrieved mint account data from RPC: {}", current_url),
                            );
    ...
    unsafe {
        ...
        if let Some(error) = init_error {
            ...
            GLOBAL_RPC_CLIENT
                .as_ref()
                .ok_or_else(|| {
                    "RPC client initialization failed: client is None after init".to_string()
                })
...
pub async fn start_rpc_stats_auto_save_service(shutdown: Arc<tokio::sync::Notify>) {
    ...
    loop {
        tokio::select! {
            _ = shutdown.notified() => {
                logger::info(LogTag::Rpc, "RPC stats auto-save service stopping");
                // Final save before shutdown
                if let Err(e) = save_global_rpc_stats() {
                    logger::error(LogTag::Rpc, &format!("Final stats save failed: {}", e));
                } else {
                    logger::info(LogTag::Rpc, "Final RPC stats saved before shutdown");
                }
                break;
            }
            _ = interval.tick() => {
    ...
    logger::info(LogTag::Rpc, "RPC stats auto-save service stopped");
}
