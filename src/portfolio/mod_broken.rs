...
impl PortfolioManager {
    ...
    pub async fn update_portfolio(
        ...
        self.current_portfolio.diversification_score = self.analyzer.calculate_diversification_score(&self.current_portfolio.positions);
    ...
    pub async fn get_rebalancing_recommendations(&self) -> BotResult<Vec<RebalanceRecommendation>> {
        ...
        for position in &self.current_portfolio.positions {
            ...
            if position_percentage > self.config.max_position_size {
                recommendations.push(RebalanceRecommendation {
                    ...
                    amount: position.current_value - (self.config.max_position_size * self.current_portfolio.total_value),
                    reason: format!("Position exceeds maximum allocation of {}%", self.config.max_position_size * 100.0),
                    priority: if position_percentage > self.config.max_position_size * 1.5 { 9 } else { 6 },
                });
            }
        }
        ...
        for position in &self.current_portfolio.positions {
            if position.pnl_percentage < -0.2 { // Down more than 20%
                recommendations.push(RebalanceRecommendation {
                    token_mint: position.token_mint,
                    ...
                    amount: 0.0,
                    reason: format!("Position down {:.1}% - review for potential exit", position.pnl_percentage * 100.0),
                    priority: 7,
                });
            }
        }
    ...
    pub fn get_top_performers(&self, limit: usize) -> Vec<&Position> {
        ...
        positions.sort_by(|a, b| b.pnl_percentage.partial_cmp(&a.pnl_percentage).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub fn get_worst_performers(&self, limit: usize) -> Vec<&Position> {
        let mut positions: Vec<&Position> = self.current_portfolio.positions.iter().collect();
        positions.sort_by(|a, b| a.pnl_percentage.partial_cmp(&b.pnl_percentage).unwrap_or(std::cmp::Ordering::Equal));
        positions.into_iter().take(limit).collect()
    }
