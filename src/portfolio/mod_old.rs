use crate::core::{
    ...
    PerformanceMetrics,
    TokenBalance,
    WalletT        self.current_portfolio.performance_metrics = self.analyzer.calculate_performance_metrics(
...
impl PortfolioManager {
    ...
    pub async fn update(
        ...
        self.update_portfolio_from_data(balances.clone(), transactions, cache).await?;
    ...
    pub async fn print_status(&self) -> BotResult<()> {
        let health = self.analyzer.analyze_portfolio_health(&self.current_portfolio.positions);
        self.display.display_portfolio_overview(&health, &self.current_portfolio.positions);
    ...
    pub async fn check_rebalancing_opportunities(&self) -> BotResult<Vec<RebalanceRecommendation>> {
        self.analyzer.generate_rebalance_recommendations(&self.current_portfolio.positions).await
    }

    /// Get portfolio health score
    pub fn get_portfolio_health_score(&self) -> PortfolioHealth {
        self.analyzer.analyze_portfolio_health(&self.current_portfolio.positions)
    }
}
