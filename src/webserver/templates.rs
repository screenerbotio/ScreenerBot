...
pub fn status_content() -> String {
    ...
    <script>
        ...
        function initStatusWebSocket() {
            if (typeof WsHub === 'undefined') {
                console.warn('[Status] WsHub not available, falling back to HTTP polling');
                return;
            }

            // Subscribe to status channel
            WsHub.subscribe('status', handleStatusUpdate);
            console.log('[Status] Subscribed to status channel');
        }
                            ...
                            <div class="status-metrics-grid">
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                        <span class="status-metric-icon">⏱️</span>
                                        <span>Uptime</span>
                                    </div>
                                ...
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                        <span class="status-metric-icon">🔥</span>
                                        <span>CPU (System)</span>
                                    </div>
                                    <div class="status-metric-value">
                                        ${formatNumber(metrics.cpu_system_percent || metrics.cpu_usage_percent || 0, 1)}<span class="status-metric-unit">%</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(metrics.cpu_process_percent || 0, 1)}<span class="status-metric-unit">%</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(metrics.system_memory_used_mb || metrics.memory_usage_mb || 0, 0)}<span class="status-metric-unit">MB</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(metrics.process_memory_mb || 0, 0)}<span class="status-metric-unit">MB</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                ...
                                <div class="status-metric-card">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(metrics.rpc_calls_total || 0, 0)}
                                    </div>
                                </div>
                            </div>
        ...
        function renderServicesRows(services) {
            ...
            return servicesList.map(({key, label}) => {
                ...
                return `
                    <tr>
                        ...
                        <td>
                            <span class="status-badge ${ready ? 'ready' : 'not-ready'}">
                                ${ready ? '✅ Ready' : '⏳ Loading'}
                            </span>
                        </td>
        ...
        function renderRPCTab(data) {
            ...
            const urlsArray = Object.entries(rpcStats.calls_per_url || {})
                .map(([url, calls]) => ({
                    ...
                    successRate: calls > 0 ? (((calls - ((rpcStats.errors_per_url || {})[url] || 0)) / calls) * 100).toFixed(2) : '100.00'
                }))
            ...
            const methodsArray = Object.entries(rpcStats.calls_per_method || {})
                .map(([method, calls]) => ({
                    ...
                    successRate: calls > 0 ? (((calls - ((rpcStats.errors_per_method || {})[method] || 0)) / calls) * 100).toFixed(2) : '100.00'
                }))
                .sort((a, b) => b.calls - a.calls);
                        ...
                        <div class="card-body">
                            <div class="status-metrics-grid">
                                ...
                                <div class="status-metric-card">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(rpcStats.total_errors, 0)}
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(rpcStats.success_rate, 2)}<span class="status-metric-unit">%</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(rpcStats.calls_per_second, 2)}
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(rpcStats.average_response_time_ms, 1)}<span class="status-metric-unit">ms</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                ...
                                <tbody>
                                    ${urlsArray.map(item => `
                                        <tr>
                                            ...
                                            <td style="text-align: right;"><strong>${formatNumber(item.calls, 0)}</strong></td>
                                            <td style="text-align: right; color: ${item.errors > 0 ? '#ef4444' : 'inherit'};">
                                                ${formatNumber(item.errors, 0)}
                                            </td>
                                            <td style="text-align: right;">
                                                <span class="status-badge ${item.successRate >= 99 ? 'ready' : 'not-ready'}">
                                                    ${item.successRate}%
                                                </span>
                                            </td>
                                        </tr>
                            ...
                            <table class="status-services-table">
                                ...
                                <tbody>
                                    ${methodsArray.slice(0, 15).map(item => `
                                        <tr>
                                            <td><code style="font-size: 0.85em;">${item.method}</code></td>
                                            <td style="text-align: right;"><strong>${formatNumber(item.calls, 0)}</strong></td>
                                            <td style="text-align: right; color: ${item.errors > 0 ? '#ef4444' : 'inherit'};">
                                                ${formatNumber(item.errors, 0)}
                                            </td>
                        ...
                        <div class="card-body">
                            <div class="status-metrics-grid">
                                <div class="status-metric-card">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(solBalance, 4)}<span class="status-metric-unit">SOL</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <table class="status-services-table">
                                        ...
                                        <tbody>
                                            ${tokenBalances.map(token => `
                                                <tr>
                                                    <td><code style="font-size: 0.85em;">${token.mint.slice(0, 8)}...${token.mint.slice(-6)}</code></td>
                                                    <td><strong>${formatNumber(token.balance_ui, 6)}</strong></td>
                                                    <td>${token.is_token_2022 ? 'Token-2022' : 'SPL Token'}</td>
                                                </tr>
                                            `).join('')}
                                        </tbody>
                                    </table>
                        ...
                        <div class="card-body">
                            <div class="status-metrics-grid">
                                <div class="status-metric-card">
                                    ...
                                    <div class="status-metric-value" style="font-size: 1.4em;">
                                        <span class="status-badge ${tradingEnabled ? 'ready' : 'not-ready'}">
                                ...
                                <div class="status-metric-card">
                                    ...
                                    <div class="status-metric-value" style="font-size: 1.4em;">
                                        <span class="status-badge ${isRunning ? 'ready' : 'not-ready'}">
                                            ${isRunning ? '✅ Yes' : '❌ No'}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="status-trader-info">
                                <div class="status-trader-row">
                                    ...
                                    <span class="status-trader-value">${tradingEnabled ? 'Enabled' : 'Disabled'}</span>
                                </div>
                                <div class="status-trader-row">
                                    <span class="status-trader-label">Runtime Status</span>
                                    <span class="status-trader-value">${isRunning ? 'Running' : 'Stopped'}</span>
                                </div>
                                <div class="status-trader-row">
                                    <span class="status-trader-label">Active Monitoring</span>
                                    <span class="status-trader-value">${isRunning ? 'Active' : 'Inactive'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <div class="status-metrics-grid">
                                ...
                                <div class="status-metric-card">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(ohlcvStats.cache_hit_rate || 0, 1)}<span class="status-metric-unit">%</span>
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                    ...
                                    <div class="status-metric-value">
                                        ${formatNumber(ohlcvStats.api_calls_per_minute || 0, 1)}
                                    </div>
                                </div>
                                <div class="status-metric-card">
                                    <div class="status-metric-header">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 12px;">
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <div style="flex: 1; height: 32px; background: var(--card-bg); border-radius: 8px; overflow: hidden; position: relative;">
                                            <div style="height: 100%; background: linear-gradient(90deg, #ef4444, #dc2626); width: ${totalTokens > 0 ? (critical / totalTokens * 100) : 0}%; transition: width 0.3s;"></div>
                                    ...
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <div style="flex: 1; height: 32px; background: var(--card-bg); border-radius: 8px; overflow: hidden; position: relative;">
                                            <div style="height: 100%; background: linear-gradient(90deg, #f59e0b, #d97706); width: ${totalTokens > 0 ? (high / totalTokens * 100) : 0}%; transition: width 0.3s;"></div>
                                    ...
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <div style="flex: 1; height: 32px; background: var(--card-bg); border-radius: 8px; overflow: hidden; position: relative;">
                                            <div style="height: 100%; background: linear-gradient(90deg, #3b82f6, #2563eb); width: ${totalTokens > 0 ? (medium / totalTokens * 100) : 0}%; transition: width 0.3s;"></div>
                                    ...
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <div style="flex: 1; height: 32px; background: var(--card-bg); border-radius: 8px; overflow: hidden; position: relative;">
                                            <div style="height: 100%; background: linear-gradient(90deg, #6b7280, #4b5563); width: ${totalTokens > 0 ? (low / totalTokens * 100) : 0}%; transition: width 0.3s;"></div>
                ...
                <div class="page-section">
                    <div class="card">
                        <div class="card-body" style="text-align: center; padding: 60px 20px;">
                            <div style="font-size: 3em; margin-bottom: 16px; opacity: 0.5;">⚠️</div>
            ...
            const servicePageController = {
                ...
                loadServices: async function(options = {}) {
                    ...
                    try {
                        ...
                        this.showServicesError('Failed to load services: ' + (error?.message || 'Network error'));
                    }
                },
                ...
                initServicesWebSocket: function() {
                    console.log('[Services] 🔌 Initializing WebSocket integration');
                    
                    // Always register cleanup when Router is available
                    const self = this;
                    if (window.Router && typeof Router.registerCleanup === 'function' && !this.cleanupRegistered) {
                        Router.registerCleanup(() => {
                            self.servicesWsBound = false;
                            self.cleanupRegistered = false;
                            console.debug('[Services] Cleanup triggered');
                            
                            // Clean up WebSocket subscriptions
                            if (typeof WsHub !== 'undefined') {
                                const { onSnapshot, onDisconnect, onReconnect } = self.wsHandlers;
                            ...
                            // Reset handlers
                            self.wsHandlers = { onSnapshot: null, onDisconnect: null, onReconnect: null };
                            self.stopServicesFallback();
                            self.clearActiveFetch('cleanup');
                        });
                    ...
                    this.wsHandlers = { onSnapshot, onDisconnect, onReconnect };
                ...
                filteredAndSortedServices: function() {
                    ...
                    const getKey = (s) => {
                        ...
                        switch (this.sortKey) {
                            ...
                            case 'health': return this.healthRank(s.health?.status);
                ...
                renderServicesTable: function() {
                    ...
                    document.getElementById('totalServices').textContent = this.servicesData.summary.total_services;
                    document.getElementById('healthyServices').textContent = this.servicesData.summary.healthy_services;
                    document.getElementById('startingServices').textContent = this.servicesData.summary.starting_services;
                    const unhealthyTotal = (this.servicesData.summary.unhealthy_services || 0) + (this.servicesData.summary.degraded_services || 0);
                    document.getElementById('unhealthyServices').textContent = unhealthyTotal;

                    // Process-wide metrics
                    if (this.servicesData.services && this.servicesData.services.length > 0) {
                        ...
                        document.getElementById('processMemory').textContent = this.formatBytes(m0.process_memory_bytes||0);
                    }
                    ...
                    const rows = filtered.map(service => {
                        ...
                        const avgPoll = this.formatDuration(m.mean_poll_duration_ns||0);
                        const pps = service.uptime_seconds>0 ? ((m.total_polls||0)/service.uptime_seconds).toFixed(2) : '0.00';

                        const taskInfo = m.task_count>0 ? `${m.task_count} tasks\nPoll: ${this.formatDuration(m.mean_poll_duration_ns)}\nIdle: ${this.formatDuration(m.mean_idle_duration_ns)}\nTotal Polls: ${m.total_polls||0}` : 'No instrumented tasks';

                        return `
                            <tr>
                                <td style="font-weight:600;">${service.name}</td>
                                <td><span class="badge ${this.getHealthBadgeClass(service.health)}">${this.getHealthStatus(service.health)}</span></td>
                                <td>${service.priority}</td>
                                <td>${service.enabled ? '✅' : '❌'}</td>
                                <td>${this.formatUptime(service.uptime_seconds)}</td>
                ...
                bindToolbarEventsOnce: function() {
                    ...
                    const self = this;
                    document.getElementById('serviceSearch')?.addEventListener('input', () => self.renderServicesTable());
                    document.getElementById('statusFilter')?.addEventListener('change', () => self.renderServicesTable());
                    document.getElementById('enabledOnly')?.addEventListener('change', () => self.renderServicesTable());
                },
                ...
                initialize: function() {
                    console.log('[Services] 🚀 Initializing page controller');
