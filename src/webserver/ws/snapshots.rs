/// Status and services snapshot types for WebSocket producers
///
/// These replace the old broadcast types.
...
/// Gather current status snapshot
pub async fn gather_status_snapshot() -> StatusSnapshot {
    ...
    // Positions (simplified - count from DB)
    ...
    let (rpc_requests_total, rpc_errors_total, rpc_stats) = if let Some(stats) = rpc_stats_opt {
        ...
        (stats.total_calls(), stats.total_errors(), Some(rpc_snapshot))
    } else {
        (0, 0, None)
    };

    // OHLCV stats (simplified - not critical)
