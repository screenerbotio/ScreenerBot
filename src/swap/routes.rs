...
impl RouteSelector {
    ...
    pub fn filter_by_minimum_output(
        &self,
        ...
        routes
            .into_iter()
            .filter(|route| { route.out_amount.parse::<u64>().unwrap_or(0) >= min_output })
            .collect()
    }
    ...
    fn create_test_route(dex: DexType, out_amount: &str, price_impact: &str) -> SwapRoute {
        SwapRoute {
            ...
            other_amount_threshold: (
                (out_amount.parse::<u64>().unwrap_or(0) * 95) /
                100
            ).to_string(),
