...
impl SwapManager {
    ...
    async fn execute_route(
        ...
        let swap_transaction = match route.dex {
            DexType::Jupiter => {
                let jupiter = self.jupiter.as_ref()
                    .ok_or_else(|| SwapError::DexNotAvailable("Jupiter not available".to_string()))?;
                jupiter.get_swap_transaction(route, &user_public_key).await?
            }
            DexType::Raydium => {
                let raydium = self.raydium.as_ref()
                    .ok_or_else(|| SwapError::DexNotAvailable("Raydium not available".to_string()))?;
                raydium.get_swap_transaction(route, &user_public_key).await?
            }
            DexType::Gmgn => {
                let gmgn = self.gmgn.as_ref()
        ...
        transaction.try_sign(&[wallet_keypair], blockhash)
            .map_err(|e| SwapError::TransactionFailed(format!("Failed to sign transaction: {}", e)))?;
        ...
        let price = if amount_tokens > 0.0 {
        ...
        if let Err(e) = self.transaction_manager.record_transaction(
            signature.to_string(),
            transaction_type,
    ...
    async fn send_transaction_with_retry(&self, transaction: &Transaction) -> Result<Signature, SwapError> {
        ...
        for attempt in 1..=self.config.retry_attempts {
            ...
            match self.rpc_manager.execute_with_fallback(move |client| {
                client.send_and_confirm_transaction(&transaction_clone).map_err(|e| anyhow::anyhow!(e))
            }).await {
                Ok(signature) => {
    ...
    fn decode_transaction(&self, transaction_data: &str) -> Result<Transaction, SwapError> {
        ...
        bincode::deserialize(&transaction_bytes)
            .map_err(|e| SwapError::SerializationError(format!("Transaction deserialize failed: {}", e)))
    }
