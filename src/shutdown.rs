...
impl ShutdownManager {
    ...
    async fn wait_for_transactions(&self) -> Result<()> {
        ...
        if remaining > 0 {
            eprintln!("⚠️ [SHUTDOWN] {} transactions still pending after {}s, forcing emergency shutdown", 
    ...
    async fn log_pending_transactions(&self) {
        ...
        if active_count > 0 {
            ...
            for (tx_id, tx) in pending.iter() {
                ...
                println!("   • {} - {:?} {} ({:.1}s elapsed)", 
                    tx_id, tx.tx_type, tx.symbol, elapsed.num_milliseconds() as f64 / 1000.0);
            }
        }
    }
    ...
    async fn check_transaction_timeouts(&self) {
        ...
        for (tx_id, tx) in timed_out {
            ...
            eprintln!("⏰ [SHUTDOWN] Transaction {} timed out after 60s: {:?} for {}", 
                tx_id, tx.tx_type, tx.symbol);
    ...
    async fn reconcile_positions(&self) -> Result<()> {
        ...
        for (mint, position) in positions.iter() {
            ...
            if diff > expected_balance * 0.01 || diff > 0.001 {
                ...
                println!("⚠️ [RECONCILE] {} balance mismatch: expected={:.9}, actual={:.9}, diff={:.9}", 
                    mint, expected_balance, actual_balance, diff);
            }
        }
        ...
        if !reconciliation_needed.is_empty() {
            println!("🔧 [RECONCILE] Updating {} positions with actual balances...", reconciliation_needed.len());
            ...
            for (mint, expected, actual) in reconciliation_needed {
                if let Some(position) = positions_write.get_mut(&mint) {
                    ...
                    println!("✏️ [RECONCILE] Updated {} balance: {:.9} → {:.9}", 
                        mint, old_amount, actual);
                }
            }
        } else {
    ...
    async fn final_cleanup(&self) {
        ...
        if let Some(start_time) = *self.shutdown_start_time.read().await {
            ...
            println!("📊 [SHUTDOWN] Total shutdown time: {:.2}s", 
                duration.num_milliseconds() as f64 / 1000.0);
        }
        ...
        println!("📊 [SHUTDOWN] Final state: {} open positions, {} closed positions", 
