...
async fn check_license_nft(mint: &Pubkey) -> Result<Option<LicenseStatus>, String> {
    ...
    logger::info(
        LogTag::License,
        &format!("Metadata PDA: {}", metadata_pda),
    );
    ...
    logger::info(
        LogTag::License,
        &format!("Fetched metadata account: {} bytes", metadata_account.data.len()),
    );
    ...
    logger::info(
        LogTag::License,
        &format!("Parsed metadata: URI={}, creators={}", metadata.uri, metadata.creators.len()),
    );
    ...
    let start_ts = parse_date_to_timestamp(&start_date)
        .map_err(|e| format!("Invalid Start Date: {}", e))?;
    let expiry_ts = parse_date_to_timestamp(&expiry_date)
    ...
    if revoked {
        logger::warning(
            LogTag::License,
            &format!("License revoked: mint={}", mint),
        );
        return Ok(None);
    }
    ...
    if now < start_ts {
        logger::warning(
            LogTag::License,
            &format!("License not yet valid: mint={}, start_ts={}, now={}", mint, start_ts, now),
        );
        return Ok(None);
    }
    ...
    if now > expiry_ts {
        logger::warning(
            LogTag::License,
            &format!("License expired: mint={}, expiry_ts={}, now={}", mint, expiry_ts, now),
        );
        return Ok(None);
    }
...
fn parse_metaplex_metadata(data: &[u8]) -> Result<MetaplexMetadata, String> {
    ...
    if has_creators {
        ...
        let creators_len = u32::from_le_bytes([data[offset], data[offset + 1], data[offset + 2], data[offset + 3]]) as usize;
        offset += 4;
