...
/// Default take-profit ceiling when no per-position override is supplied.
pub const DEFAULT_TARGET_MAX_PERCENT: f64 = 38.0; // decoupled from INSTANT_EXIT_LEVEL_1 so momentum logic can breathe
...
const QUICK_WINDOWS: &[(f64, f64)] = &[
    (3.0, 40.0), // 3 minutes: 40% profit
    (4.5, 35.0), // 4.5 minutes: 35% profit (bridges early spike gap)
    (6.0, 55.0), // 6 minutes: 55% profit
...
pub fn continuation_odds(profit_percent: f64, minutes_held: f64) -> f64 {
    ...
    if profit_percent <= 0.0 {
        let severity = (-profit_percent).min(60.0);
        let adjusted = 0.55 - severity * 0.0035; // down to ~0.34 at -60%
        return adjusted.clamp(0.35, 0.55);
    }
...
pub async fn should_sell(position: &Position, current_price: f64) -> bool {
    ...
    let mut target_max = position.profit_target_max
        .unwrap_or(DEFAULT_TARGET_MAX_PERCENT)
        .clamp(1.0, 500.0);
    ...
    let weak_peak = peak_profit < target_min + 3.0;
    if weak_peak {
        let trigger =
            (minutes_held >= 6.0 && pnl_percent <= -18.0) ||
            (minutes_held >= 12.0 && pnl_percent <= -22.0) ||
            (minutes_held >= 20.0 && pnl_percent <= -28.0) ||
        ...
        if trigger {
            if is_debug_profit_enabled() {
                log(
                    ...
                    "LOSS_SOFT_STOP",
                    &format!(
                        "{} pnl={:.2}% peak={:.2}% t={:.1}m (weak peak soft stop)",
                        position.symbol,
                        pnl_percent,
                        peak_profit,
                        minutes_held
        ...
        peak_profit < target_min &&
        peak_profit - pnl_percent >= 12.0 &&
        pnl_percent <= -15.0 &&
        minutes_held >= 10.0
    {
        if is_debug_profit_enabled() {
            log(
                LogTag::Profit,
                "LOSS_SOFT_STOP_WEAK_PEAK",
                &format!(
                    "{} pnl={:.2}% peak={:.2}% dd={:.2}% t={:.1}m",
                    position.symbol,
                    pnl_percent,
                    peak_profit,
    ...
    if pnl_percent < 0.0 {
        exit_score += (-pnl_percent).min(25.0) * 0.03; // up to +0.75 for deep red positions
    }
    if pnl_percent >= INSTANT_EXIT_LEVEL_1 {
        exit_score += 1.0;
    }
    ...
    let raw_multiplier = learning.get_exit_score_adjustment(
        &position.mint,
        current_price,
        entry,
        minutes_held as u32
