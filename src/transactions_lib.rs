...
impl TransactionsManager {
    ...
    async fn integrate_token_information(
        ...
        match get_pool_price(&token_mint) {
            Some(price_info) => {
    ...
    pub async fn extract_basic_transaction_info(
        ...
        if let Some(raw_data) = &transaction.raw_transaction_data {
            let wallet_str = self.wallet_pubkey.to_string();
            let mut wallet_account_index: Option<usize> = None;
            ...
            if let Some(meta) = raw_data.get("meta") {
                ...
                if wallet_account_index.is_none() {
                    if let Some(transaction_json) = raw_data.get("transaction") {
                        if let Some(message) = transaction_json.get("message") {
                            if
                                let Some(account_keys) = message
                                    .get("accountKeys")
                                    .and_then(|v| v.as_array())
                            {
                                wallet_account_index = account_keys.iter().position(|key| {
                                    key.as_str()
                                        .map(|value| value == wallet_str)
                                        .unwrap_or(false)
                                });

                                if let Some(header) = message.get("header") {
                                    if
                                        let Some(num_required_signatures) = header
                                            .get("numRequiredSignatures")
                                            .and_then(|v| v.as_u64())
                                    {
                                        if let Some(idx) = wallet_account_index {
                                            transaction.wallet_signed =
                                                idx < (num_required_signatures as usize);
                                        }
                                    }
                                }
                            }
                    ...
                    )
                {
                    if let Some(idx) = wallet_account_index {
                        if
                            let (Some(pre_balance_lamports), Some(post_balance_lamports)) = (
                                pre_balances.get(idx).and_then(|v| v.as_i64()),
                                post_balances.get(idx).and_then(|v| v.as_i64()),
                            )
                        {
                            let balance_change_lamports =
                                post_balance_lamports - pre_balance_lamports;
                            transaction.wallet_lamport_change = balance_change_lamports;
