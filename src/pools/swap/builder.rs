...
impl SwapBuilder {
    ...
    pub async fn execute(request: SwapRequest) -> Result<SwapResult, SwapError> {
        ...
        match program_kind {
            ProgramKind::RaydiumCpmm => {
                RaydiumCpmmSwap::execute_swap(request, pool_data).await
            }
            _ => Err(SwapError::InvalidPool(
                format!("Unsupported program: {:?}", program_kind)
            ))
        }
    }
        ...
        match request.direction {
            SwapDirection::Buy => {
                if request.amount < 0.001 {
                    return Err(SwapError::InvalidInput("SOL amount too small (minimum 0.001 SOL)".to_string()));
                }
            }
            SwapDirection::Sell => {
                if request.amount < 1.0 {
                    return Err(SwapError::InvalidInput("Token amount too small (minimum 1 token)".to_string()));
                }
            }
        }
    ...
    async fn fetch_pool_data(
        ...
        let program_kind = match account_data.owner.to_string().as_str() {
            ...
            _ => return Err(SwapError::InvalidPool(
