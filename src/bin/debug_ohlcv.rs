...
async fn test_fetch(mint: &str, timeframe_str: &str, limit: usize) {
    ...
    match ohlcvs::get_ohlcv_data(mint, timeframe, None, limit, None, None).await {
        Ok(data) => {
            if data.is_empty() {
                println!("⚠️  No data found in database");
                println!();
...
async fn test_pools(mint: &str) {
    ...
    match ohlcvs::get_available_pools(mint).await {
        Ok(pools) => {
            if pools.is_empty() {
                println!("⚠️  No pools found on GeckoTerminal");
                println!();
            ...
            match ohlcvs::has_data(mint).await {
                Ok(has_data) => {
                    if has_data {
                            ...
                            match ohlcvs::get_data_gaps(mint, timeframe).await {
                                Ok(gaps) => {
                                    if gaps.is_empty() {
                                        ...
                                        println!("  {} - ⚠️  {} gap(s) found", tf_str, gaps.len());
                                    }
                                }
                                Err(_) => {
                        ...
                        println!("⚠️  No data found for this token");
                        println!();
                    ...
                    Ok(api_response) => {
                        if let Some(data) = api_response.data {
                            ...
                            if error_count == 0 {
                                ...
                                println!("⚠️  Some candles failed validation");
                            }
                        } else {
