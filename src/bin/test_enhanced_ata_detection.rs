...
async fn test_ata_detection_for_transaction(
    ...
    let configs = read_configs("configs.json").map_err(|e|
        anyhow::anyhow!("Failed to load configs: {}", e)
    )?;

    // Create transaction fetcher
    let fetcher = transactions::fetcher::TransactionFetcher
        ::new(&configs, None)
        .map_err(|e| anyhow::anyhow!("Failed to create fetcher: {}", e))?;

    // Fetch transaction
    println!("📡 Fetching transaction from chain...");
    let transaction = fetcher
        .get_transaction_details(signature, "wallet_placeholder").await
    ...
    if let Some(tx_details) = transaction {
        ...
        for (i, (pre_balance, post_balance)) in pre_balances
            .iter()
        ...
        match token_name {
            "PHI" => {
                if ata_detected {
                    println!(
                        "  ✅ CORRECT: PHI transaction correctly identified as having ATA contamination"
                    );
                } else {
                    println!(
                        "  ❌ ERROR: PHI transaction should have been flagged for ATA contamination!"
                    );
                }
            }
            "AIRFRY" => {
                if !ata_detected {
                    ...
                    println!(
                        "  ❌ ERROR: AIRFRY transaction should NOT have been flagged for ATA!"
                    );
                }
            }
            _ => {}
