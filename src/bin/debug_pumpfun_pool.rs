...
use screenerbot::constants::{PUMP_FUN_AMM_PROGRAM_ID, PUMP_FUN_LEGACY_PROGRAM_ID, SOL_MINT, SYSTEM_PROGRAM_ID, SOL_DECIMALS};
...
fn main() {
    ...
    if let Some(v) = read_u64("PUMPDEBUG_VIRT_TOKEN") { known_u64s.push(("virtual_token_reserves".to_string(), v)); }
    if let Some(v) = read_u64("PUMPDEBUG_VIRT_SOL") { known_u64s.push(("virtual_sol_reserves".to_string(), v)); }
    if let Some(v) = read_u64("PUMPDEBUG_REAL_TOKEN") { known_u64s.push(("real_token_reserves".to_string(), v)); }
    if let Some(v) = read_u64("PUMPDEBUG_REAL_SOL") { known_u64s.push(("real_sol_reserves".to_string(), v)); }
    if let Some(v) = read_u64("PUMPDEBUG_TOTAL_SUPPLY") { known_u64s.push(("token_total_supply".to_string(), v)); }

    let creator_from_env = std::env::var("PUMPDEBUG_CREATOR_PUBKEY").ok().and_then(|s| Pubkey::from_str(&s).ok());
    ...
    for off in 0..=data.len().saturating_sub(8) {
        let val = u64::from_le_bytes([
            data[off], data[off+1], data[off+2], data[off+3], data[off+4], data[off+5], data[off+6], data[off+7]
        ]);
    ...
    for (label, target) in &known_u64s {
        let mut matches: Vec<(usize, u64)> = u64_hits.iter().cloned().filter(|(_, v)| v == target).collect();
        matches.sort_by_key(|(o, _)| *o);
        if matches.is_empty() {
            println!("  ⚠️  No match for {} = {}", label, target);
        } else {
            println!("  ✓ {} candidates for {} = {}:", matches.len(), label, target);
            for (i, (off, val)) in matches.iter().enumerate() { println!("    - [{}] offset {} (val {})", i, off, val); }
    ...
    if let Some(creator_pk) = creator_from_env {
        ...
        match creator_hit {
            ...
            None => println!("  ⚠️  Creator pubkey not found in data: {}", creator_pk),
        }
    }
    ...
    if labeled_hits.len() >= 2 {
        ...
        for (label, off, val) in &labeled_hits { println!("  - {:>20} @ {:>3} = {}", label, off, val); }
        ...
        println!("  Cluster span: [{}..{}) ({} bytes)\n", span_start, span_end, span_end.saturating_sub(span_start));
    }

    // Optional: search likely bools (0x00/0x01) near cluster
    if !labeled_hits.is_empty() {
        let center = labeled_hits.iter().map(|(_, off, _)| *off).sum::<usize>() / labeled_hits.len();
        let start = center.saturating_sub(24);
        let end = (center + 24).min(data.len());
        let mut bool_candidates: Vec<(usize, u8)> = Vec::new();
        for i in start..end { if data[i] == 0 || data[i] == 1 { bool_candidates.push((i, data[i])); } }
        if !bool_candidates.is_empty() {
            println!("  Nearby bool candidates around center {} (±24 bytes):", center);
            for (off, b) in &bool_candidates { println!("    - offset {} value {}", off, b); }
        }
    }
    ...
    let dump_window = |off: usize, label: &str| {
        ...
        print!("  [{}] hex @{}: ", label, off);
        for i in s..e { print!("{:02x} ", data[i]); }
        println!();
    };
    for (label, off, _) in &labeled_hits { dump_window(*off, label); }
    if let Some(off) = creator_hit { dump_window(off, "creator"); }
    ...
    for (off, pk) in &found_pubkeys {
        ...
        match rpc_client.get_account(pk) {
            Ok(account) => {
                ...
                if owner == "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA" && account.data.len() >= 82 {
                    ...
                    println!("✓ TOKEN MINT (decimals={}, supply={}, initialized={})", 
                ...
                else if owner == "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA" && account.data.len() >= 165 {
    ...
    if let Some((mint_offset, mint_pk, decimals)) = token_mint {
        ...
        println!("═════════════════════════════════════════════════════════════════════════════");
        println!();
        println!("Token Mint: {} (offset {}, decimals {})", mint_pk, mint_offset, decimals);
        println!();
        
        // Use the discovered reserve offsets
        let virt_token = labeled_hits.iter().find(|(l, _, _)| l == "virtual_token_reserves").map(|(_, _, v)| *v);
        let virt_sol = labeled_hits.iter().find(|(l, _, _)| l == "virtual_sol_reserves").map(|(_, _, v)| *v);
        let real_token = labeled_hits.iter().find(|(l, _, _)| l == "real_token_reserves").map(|(_, _, v)| *v);
        let real_sol = labeled_hits.iter().find(|(l, _, _)| l == "real_sol_reserves").map(|(_, _, v)| *v);
        
        if let (Some(vt), Some(vs), Some(rt), Some(rs)) = (virt_token, virt_sol, real_token, real_sol) {
            ...
            if token_amount > 0.0 {
                ...
                if token_amount_real > 0.0 {
                    ...
                    println!("Price (using real reserves): {:.15} SOL per token", price_sol_real);
                    println!();
                }
            } else {
            ...
            println!("⚠️  Missing reserve fields - cannot calculate price");
        }
    } else {
        println!("⚠️  No token mint found in account data - cannot calculate price");
    }
    ...
    if data.len() >= 200 {
        ...
        println!("⚠️  Data length ({} bytes) is less than 200 bytes", data.len());
...
fn analyze_mint(rpc_client: &RpcClient, mint_pubkey: &Pubkey) {
    ...
    if mint_str == SYSTEM_PROGRAM_ID {
        ...
        println!("  Status: ⚠️  This is the System Program, not a token mint!");
        return;
    }
    ...
    match rpc_client.get_account(mint_pubkey) {
        Ok(account) => {
            ...
            if account.owner.to_string() == "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA" {
                ...
                println!("  Status: ⚠️  Not a standard SPL token mint");
            }
        }
        Err(e) => {
...
fn analyze_token_account(rpc_client: &RpcClient, account_pubkey: &Pubkey) {
    ...
    if account_str == SYSTEM_PROGRAM_ID {
        println!("  Type: System Program");
        println!("  Status: ⚠️  This is the System Program, not a token account!");
        return;
    }
    ...
    match rpc_client.get_account(account_pubkey) {
        Ok(account) => {
            ...
            if account.owner.to_string() == "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA" {
                ...
                println!("  Status: ⚠️  Not a standard SPL token account");
            }
        }
        Err(e) => {
