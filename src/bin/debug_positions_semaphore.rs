...
async fn main() -> Result<(), String> {
    ...
    log(LogTag::Positions, "INFO", &format!("Open positions: {} / {} (semaphore reconciliation implicit)", open, max));
    if arguments::is_debug_positions_enabled() {
        log(LogTag::Positions, "DEBUG", &format!("Enabled debug modes: {:?}", arguments::get_enabled_debug_modes()));
    }
    Ok(())
}
