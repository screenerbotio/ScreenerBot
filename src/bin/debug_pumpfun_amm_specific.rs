...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();
    set_cmd_args(
        vec!["debug_pumpfun_amm_specific".to_string(), "--debug-pool-decoders".to_string()]
    );
    ...
    println!("Owner is PumpFun AMM: {}", if pool_acc.owner.to_string() == PUMP_FUN_AMM_PROGRAM_ID {
        "✅"
    } else {
        "❌"
    });
    ...
    if pool_acc.data.len() >= 250 {
        ...
        let (sol_mint, token_mint, sol_vault, token_vault) = if
            let (Some(base), Some(quote), Some(v1), Some(v2)) = (
                base_mint.as_ref(),
                quote_mint.as_ref(),
                vault1.as_ref(),
                vault2.as_ref(),
