...
use screenerbot::{ learner, logger::{ init_file_logging, log, LogTag } };
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match learner::initialize_learning_system().await {
        Ok(_) => {
            log(LogTag::Learning, "INFO", "✅ Learner initialization test passed");
    ...
    let adjustment = integration.get_entry_confidence_adjustment(mint, 0.001, 10.0, 5.0).await;
    println!("✅ Entry confidence adjustment: PASSED (result: {:.3})", adjustment);

    // Test 4: Test exit score adjustment (simulated)
    let exit_adjustment = integration.get_exit_score_adjustment(mint, 0.002, 0.001, 5).await;
    println!("✅ Exit score adjustment: PASSED (result: {:.3})", exit_adjustment);

    // Test 5: Check if predictions are available
    let predictions_available = learner::learning_predictions_available().await;
    println!("✅ Predictions availability check: PASSED (available: {})", predictions_available);
    ...
    log(LogTag::Learning, "INFO", "All learner integration tests completed successfully");
