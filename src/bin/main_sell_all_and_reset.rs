...
use screenerbot::utils::{
    ...
    get_wallet_address, safe_truncate,
};
use solana_sdk::{
...
fn print_help() {
    ...
    log(
        LogTag::System,
        "INFO",
        "    • data/configs.json (wallet keys and RPC endpoints)",
    );
    log(
        LogTag::System,
        "INFO",
...
async fn burn_single_token_amount(account: &TokenAccountInfo) -> Result<String, String> {
    ...
    let configs = screenerbot::configs::read_configs()
        .map_err(|e| format!("Failed to read configs: {:?}", e))?;
    let wallet_keypair = screenerbot::configs::load_wallet_from_config(&configs)
