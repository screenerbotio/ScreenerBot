...
async fn burn_dust_tokens_with_retry(
    ...
    for account in &burnable_accounts {
        log(
            LogTag::System,
            "BURN_ATTEMPT",
            &format!("Attempting to burn {} raw units of {}", account.balance, &account.mint)
        );
        ...
        match burn_single_token_amount(account).await {
            ...
            Err(error_msg) => {
                log(
                    LogTag::System,
                    "BURN_FAILED",
                    &format!("Failed to burn dust amount for {}: {}", &account.mint, error_msg)
                );
                failed_burns += 1;
            }
        }
...
async fn burn_single_token_amount(account: &TokenAccountInfo) -> Result<String, String> {
    ...
    log(
        LogTag::System,
        "BURN_START",
        &format!("Starting burn of {} raw units for token {}", account.balance, &account.mint)
    );
...
async fn sell_single_token_with_retry(
    ...
    if dry_run {
        log(
            LogTag::System,
            "DRY_SELL",
            &format!("Would sell {} raw units of {}", account.balance, &account.mint)
        );
    ...
    log(
        LogTag::System,
        "SELL_START",
        &format!("Starting sell for token: {} ({} raw units)", &account.mint, account.balance)
    );
    ...
    for attempt in 1..=MAX_SELL_RETRIES {
        log(
            LogTag::System,
            "SELL_ATTEMPT",
            &format!("Sell attempt {} of {} for token {}", attempt, MAX_SELL_RETRIES, &account.mint)
        );
        ...
        match attempt_single_sell(account).await {
            ...
            Err(error_msg) => {
                log(
                    LogTag::System,
                    "SELL_ATTEMPT_FAILED",
                    &format!("Sell attempt {} failed for {}: {}", attempt, &account.mint, error_msg)
                );
                ...
                if attempt < MAX_SELL_RETRIES {
                    ...
                    log(
                        LogTag::System,
                        "SELL_FAILED",
                        &format!("All sell attempts failed for {}: {}", &account.mint, error_msg)
                    );
                    return (account.mint.clone(), false, Some(error_msg));
                }
            }
...
async fn attempt_single_sell(account: &TokenAccountInfo) -> Result<String, String> {
    ...
    if swap_result.success {
        ...
        Ok(format!("Successfully sold {} for {:.6} SOL", &account.mint, sol_amount))
    } else {
...
async fn close_single_ata_with_retry(
    ...
    if dry_run {
        log(LogTag::System, "DRY_ATA", &format!("Would close ATA for token: {}", &account.mint));
        return (account.mint.clone(), true, Some("DRY_RUN_TX".to_string()));
    }

    log(LogTag::System, "ATA_START", &format!("Starting ATA close for token: {}", &account.mint));
        ...
        match attempt_single_ata_close(&account.mint, wallet_address, recently_processed).await {
            Ok(signature) => {
                log(
                    LogTag::System,
                    "ATA_SUCCESS",
                    &format!("Successfully closed ATA for {}. TX: {}", &account.mint, signature)
                );
