...
use screenerbot::transactions::{
    ...
    get_transactions_with_retry_and_fallback,
    transaction_involves_token,
...
fn format_token_mint(mint: &str) -> String {
...
fn estimate_token_price(analyses: &[(&SignatureInfo, TransactionAnalysis)], token_mint: &str, token_amount: f64) -> String {
    // Find the transaction that contains this token change
    for (_, analysis) in analyses {
        let has_token_change = analysis.token_changes.iter().any(|change| change.mint == token_mint);
        if !has_token_change {
            continue;
        }
        ...
        if let Some(sol_change) = analysis.token_changes.iter().find(|change| change.mint == sol_mint) {
...
fn format_change_type(change_type: &TokenChangeType) -> String {
    match change_type {
        TokenChangeType::Increase => "📈 INCREASE".to_string(),
        TokenChangeType::Decrease => "📉 DECREASE".to_string(),
        TokenChangeType::NoChange => "➖ NO CHANGE".to_string(),
    }
}

fn format_program_name(program_id: &str) -> String {
    match program_id {
        "whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc" => "🌊 Whirlpool".to_string(),
        "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM" => "🐋 Orca".to_string(),
        "DjVE6JNiYqPL2QXyCUUh8rNjHrbz9hXHNYt99MQ59qw1" => "🐋 Orca V2".to_string(),
        "srmqPioktnPG6V2eRJoGnR7X6Qe6hR6R7KrMxRn5dFx" => "📊 Serum".to_string(),
        "CAVu6WCU6h4eBZo2jNTHJrmKTUYjfyBvVTBSX3fPpGm" => "🕳️ Cavern".to_string(),
        _ => format_token_mint(program_id),
    }
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let signatures = match get_recent_signatures_with_fallback(&client, &wallet_address, &configs, TRANSACTION_LIMIT).await {
        Ok(sigs) => {
    ...
    // Step 2: Fetch and analyze transaction details
    println!("🔍 Fetching and analyzing transaction details...");
    let transactions = get_transactions_with_retry_and_fallback(&client, &signatures, &configs, Some(TRANSACTION_LIMIT)).await;
    ...
    let significant_transactions: Vec<_> = analyses.iter()
        .filter(|(_, analysis)| {
    ...
    if !significant_transactions.is_empty() {
        ...
        let summary_data: Vec<TransactionSummary> = significant_transactions.iter().enumerate().map(|(i, (sig_info, analysis))| {
            TransactionSummary {
                index: format!("{}", i + 1),
                time: format_timestamp(analysis.block_time),
                status: if analysis.is_success { "✅ Success" } else { "❌ Failed" }.to_string(),
                ...
                signature: format!("{}...{}", &analysis.signature[..8], &analysis.signature[analysis.signature.len()-8..]),
            }
        }).collect();
    ...
    if !all_token_changes.is_empty() {
        ...
        let token_changes_data: Vec<TokenChangeSummary> = all_token_changes.iter().map(|(sig_info, change)| {
            let change_type = if change.change > 0.0 { "📈 BUY/RECEIVE" } else { "📉 SELL/SEND" };
            ...
            TokenChangeSummary {
                ...
                amount,
                estimated_price,
                signature: format!("{}...{}", &sig_info.signature[..8], &sig_info.signature[sig_info.signature.len()-8..]),
            }
        }).collect();
    ...
    if !all_swaps.is_empty() {
        ...
        let swap_data: Vec<SwapSummary> = all_swaps.iter().enumerate().map(|(i, (sig_info, swap))| {
    ...
    println!("   ✅ Successful transactions: {}", analyses.iter().filter(|(_, a)| a.is_success).count());
    println!("   ❌ Failed transactions: {}", analyses.iter().filter(|(_, a)| !a.is_success).count());
    println!("   🔄 Transactions with swaps: {}", all_swaps.len());
    println!("   💰 Significant token changes: {}", all_token_changes.len());
    println!("   💸 Total fees paid: {:.6} SOL", analyses.iter().map(|(_, a)| a.fee_sol).sum::<f64>());
