...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    log(LogTag::Pool, "TEST_PRIORITY", &format!("✅ Added {} to priority tokens", &test_token[..8]));
    ...
    log(LogTag::Pool, "TEST_WATCHLIST", &format!("✅ Added {} tokens to watchlist", watchlist_tokens.len()));
    
    // Check watchlist status
    let (total, never_updated, _last_update) = get_watchlist_status().await;
    log(LogTag::Pool, "TEST_WATCHLIST", &format!("✅ Watchlist status: {} total, {} never updated", total, never_updated));
    ...
    if let Some(price_result) = get_price(test_token, None, false).await {
        log(LogTag::Pool, "TEST_PRICE", &format!("✅ Got price for priority token: {:.8} SOL", price_result.price_sol.unwrap_or(0.0)));
    } else {
    ...
    if let Some(price_result) = get_price(&watchlist_tokens[0], None, false).await {
        log(LogTag::Pool, "TEST_PRICE", &format!("✅ Got price for watchlist token: {:.8} SOL", price_result.price_sol.unwrap_or(0.0)));
    } else {
    ...
    log(LogTag::Pool, "TEST_CLEANUP", &format!("✅ Removed {} from priority tokens", &test_token[..8]));
