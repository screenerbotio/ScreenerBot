...
fn extract_raydium_vaults(data: &[u8]) -> Result<(Pubkey, Pubkey), Box<dyn std::error::Error>> {
    ...
    let vault_0 = Pubkey::new_from_array(
        vault_0_bytes.try_into().map_err(|_| "Failed to convert vault_0 bytes")?
    );
    offset += 32;

    let vault_1_bytes = &data[offset..offset + 32];
    let vault_1 = Pubkey::new_from_array(
        vault_1_bytes.try_into().map_err(|_| "Failed to convert vault_1 bytes")?
    );
...
fn extract_raydium_cpmm_vaults(
    data: &[u8]
) -> Result<(Pubkey, Pubkey), Box<dyn std::error::Error>> {
    if data.len() < 8 + 32 * 10 {
...
async fn get_vault_balances(
    ...
    let vault_a_account = accounts[0].as_ref().ok_or("Vault A account not found")?;
    let vault_b_account = accounts[1].as_ref().ok_or("Vault B account not found")?;
    ...
    let balance_a = u64::from_le_bytes(
        vault_a_account.data[64..72].try_into().map_err(|_| "Failed to read vault A balance")?
    );
    let balance_b = u64::from_le_bytes(
        vault_b_account.data[64..72].try_into().map_err(|_| "Failed to read vault B balance")?
    );
...
async fn main() {
    ...
    match decoders::decode_pool(program_kind, &accounts, &args.token_mint, &args.quote_mint) {
        Some(price_result) => {
        ...
        None => {
            ...
            if args.verbose {
                log(LogTag::PoolCalculator, "ERROR", "decoder returned None");
            }
        }
    }
}
