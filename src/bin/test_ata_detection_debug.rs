...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match fetcher.get_transaction_details_with_fallback(signature).await {
            Ok(Some(transaction_result)) => {
                ...
                if let Some(meta) = &details.meta {
                    ...
                    let balance_changes: Vec<i64> = meta.post_balances
                        .iter()
                    ...
                    if !balance_changes.is_empty() {
                        ...
                        println!(
                            "💰 Wallet SOL Change: {} lamports ({:.9} SOL)",
                            wallet_sol_change,
                            (wallet_sol_change as f64) / 1_000_000_000.0
                        );
                        ...
                        if wallet_sol_change == expected_lamports {
                            ...
                            println!(
                                "  ⚠️  Different from expected: {} vs {} lamports",
                                wallet_sol_change,
                                expected_lamports
                            );
                        }
                ...
                println!(
                    "  • ATA Rent Amount: {} lamports ({:.9} SOL)",
                    ata_rent_amount,
                    (ata_rent_amount as f64) / 1_000_000_000.0
                );
                println!(
                    "  • SOL from Trade Only: {} lamports ({:.9} SOL)",
                    sol_from_trade_only,
                    (sol_from_trade_only as f64) / 1_000_000_000.0
                );
    ...
    println!("\n📋 SUMMARY:");
    println!(
        "• PHI transaction shows suspiciously high SOL (0.002554144) - likely includes ATA rent"
    );
