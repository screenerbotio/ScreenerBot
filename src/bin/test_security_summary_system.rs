...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    log(LogTag::System, "TEST", "Starting security summary system test");
    ...
    if let Some(analyzer) = get_security_analyzer() {
        log(LogTag::System, "TEST", "Testing token analysis to generate metrics...");
        ...
        for token in &test_tokens {
            log(LogTag::System, "TEST", &format!("Analyzing token: {}", token));
        ...
        log(LogTag::System, "TEST", "Waiting for summary reports (90 seconds to see 3 reports)...");
    ...
    log(LogTag::System, "TEST", "Security summary system test completed");
    Ok(())
}
