...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialize logging
    init_file_logging();
    log(LogTag::System, "START", "🧪 Testing SOL price coverage logic with debug enabled");
    ...
    if let Err(e) = init_ohlcv_database() {
        log(LogTag::System, "ERROR", &format!("Failed to initialize OHLCV database: {}", e));
        return Err(e.into());
    }
    ...
    if let Err(e) = init_ohlcv_service().await {
        log(LogTag::System, "ERROR", &format!("Failed to initialize OHLCV service: {}", e));
        return Err(e.into());
    }

    // Test SOL price coverage requirement with timeout
    log(LogTag::System, "TEST1", "🔍 Testing SOL price coverage requirement...");
    ...
    match coverage_result {
        Ok(Ok(())) => {
            log(
                LogTag::System,
                "SUCCESS",
                &format!("✅ SOL coverage available for {} hours", MAX_OHLCV_AGE_HOURS)
            );

            // Test token OHLCV fetching now that coverage is ensured
            log(LogTag::System, "TEST2", "🔍 Testing token OHLCV fetching with coverage...");
            ...
            match ohlcv_result {
                ...
                Ok(Err(e)) => {
                    log(LogTag::System, "INFO", &format!("Token OHLCV failed: {}", e));
                }
                Err(_) => {
                    log(LogTag::System, "TIMEOUT", "⏰ Token OHLCV request timed out");
                }
            }
        }
        Ok(Err(e)) => {
            log(LogTag::System, "FAIL", &format!("❌ SOL coverage check failed: {}", e));
            println!("SOL coverage test failed as expected if no SOL price data: {}", e);
            ...
            match ohlcv_result {
                ...
                Ok(Err(e)) => {
                    log(
                        LogTag::System,
                        "SUCCESS",
                        &format!("✅ Token OHLCV properly blocked without SOL coverage: {}", e)
                    );
                }
                Err(_) => {
                    log(LogTag::System, "TIMEOUT", "⏰ Token OHLCV request timed out (expected)");
                }
            }
        }
        Err(_) => {
