...
pub async fn display_token_pool_prices(token_mint: &str) -> Result<()> {
    ...
    let configs = read_configs("configs.json")
        .map_err(|e| anyhow::anyhow!("Failed to read configs: {}", e))?;
    ...
    match pool_discovery.get_token_pool_prices(token_mint).await {
        Ok(pool_results) => {
            ...
            for (index, pool) in pool_results.iter().enumerate() {
                ...
                println!("   {} {}", 
                    "📍 Pool Address:".dimmed(), 
                    pool.pool_address.bright_cyan()
                );
                
                println!("   {} ${:.2}", 
                    "💧 Liquidity USD:".bright_blue(), 
                    pool.liquidity_usd
                );
                
                println!("   {} ${:.2}", 
                    "📈 24h Volume:".bright_green(), 
                    pool.volume_24h
                );
                ...
                if pool.calculation_successful && pool.calculated_price > 0.0 {
                    ...
                    if pool.dexscreener_price > 0.0 {
                        ...
                        println!("   {} {}%", 
                            "📊 Price Difference:".bright_cyan(), 
                            match diff_color {
                                "bright_red" => format!("{:.2}", pool.price_difference_percent).bright_red(),
                                "bright_yellow" => format!("{:.2}", pool.price_difference_percent).bright_yellow(),
            ...
            // Summary
            let supported_count = pool_results.iter().filter(|p| p.calculation_successful).count();
            let unsupported_count = pool_results.len() - supported_count;
            
            println!("{}", "=".repeat(80).bright_cyan());
            println!("{}", "📋 SUMMARY".bright_green().bold());
            println!("{}", "=".repeat(80).bright_cyan());
            println!("   {} {} pools", "📊 Total:".bright_blue(), pool_results.len().to_string().bright_white().bold());
            println!("   {} {} pools", "✅ Fully Supported:".bright_green(), supported_count.to_string().bright_green().bold());
            println!("   {} {} pools", "⚠️  API Only:".bright_yellow(), unsupported_count.to_string().bright_yellow().bold());
            
            if let Some(best_pool) = pool_results.first() {
                println!();
                println!("   {} {} ({}) with ${:.2} liquidity", 
                    "🏆 Best Pool:".bright_yellow().bold(),
                    format!("{}/{}", best_pool.token_a_symbol, best_pool.token_b_symbol).bright_white().bold(),
...
async fn main() -> Result<()> {
    ...
    if args.len() != 2 {
        println!("{}", "❌ Usage: cargo run --bin tool_pool_prices <token_mint_address>".bright_red().bold());
        println!();
